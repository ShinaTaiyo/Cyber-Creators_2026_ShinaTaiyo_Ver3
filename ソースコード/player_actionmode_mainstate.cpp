...
void CPlayerActionMode_PrepDive::Process(CPlayer* pPlayer)
{
	...
	{//ワイヤー発射移動モードにチェンジ
		pPlayer->ChengeActionMode(DBG_NEW CPlayerActionMode_WireShot(pPlayer));
	}
...
CPlayerActionMode_WireShot::CPlayerActionMode_WireShot(CPlayer* pPlayer)
{
	...
	float fYaw = atan2f(MathRot.x, MathRot.z);//Z（前）を軸にする
...
{
	CObject* pObj = CObject::GetTopObject(static_cast<int>(CObject::TYPE::ENEMY));//敵のリストの戦闘オブジェクトを取得
	CLockon* pLockOn = pPlayer->GetLockOn();//ロックオンを取得
...
void CPlayerActionMode_WireShot::DecisionCameraRot(CPlayer* pPlayer)
{
    ...
    //カメラの向きを求める
    //==========================
    D3DXVECTOR3 ComRot = pWireHead->GetPosInfo().GetPos() - pPlayer->GetPosInfo().GetPos();//ワイヤーの頭とプレイヤーのベクトルを元めっる
    D3DXVec3Normalize(&ComRot, &ComRot);//上記で求めたベクトルを正規化する

	//正規化ベクトルをもとにワイヤーの頭とプレイヤーの向きを求める
    float fYaw = atan2f(ComRot.x, ComRot.z);//Z方向（前）を軸にYawを求める
...
{
	CObjectX::MoveInfo& PlayerMoveInfo = pPlayer->GetMoveInfo();
...
void CPlayerActionMode_DiveMove::Process(CPlayer* pPlayer)
{
	...
	{//ダイブ時に判定したら移動モードと攻撃モードを通常に戻す
		...
		{//引っ付き→ダイブ（もしワイヤーヘッドが衝突したオブジェクトが敵の場合、敵には引っ付きたくないので、攻撃に移行する
			...
			pPlayer->ChengeActionMode(DBG_NEW CPlayerActionMode_Stuck(pPlayer));//ひっ付きモードに変える
		}
		else
		{//攻撃→ダイブモード
			pWire->SetUseDraw(false);//ワイヤーの頭を描画しない
			PlayerRotInfo.SetRot(D3DXVECTOR3(0.0f, pCamera->GetRot().y, 0.0f));
			pPlayer->ChengeActionMode(DBG_NEW CPlayerActionMode_DiveAttack(pPlayer));//ダイブ攻撃モードに変更
		}
	}
}
//=========================================================================================================================================================
...
void CPlayerActionMode_Stuck::Process(CPlayer* pPlayer)
{
	...
	{//引っ付きながら射撃する
		...
		{//m_bStartWireShotをtrueにしたときにステートが変わらないようにするため、falseの時だけステートを変える
			pPlayer->ChengeAttackMode(DBG_NEW CPlayerAttack_Shot());
