...
void CObject2D::Update()
{
	...
	m_rot.z += m_fPolygonRotSpeed * GetDeltaTimeScale(this);//ポリゴンを回転させる
...
{	
	...
	{
		m_Scale += m_AddScale * GetDeltaTimeScale(this);//拡大率を加算する
	}
...
void CObject2D::FloatingProcess()
{
	if (m_bUseFloating == true)
	{
		...
		{//浮遊速度の最大に達したら移動方向を逆にする
      	...
      	m_fFloatingSpeed += m_fFloatingAddSpeed * GetDeltaTimeScale(this);//速度を増やし続ける
