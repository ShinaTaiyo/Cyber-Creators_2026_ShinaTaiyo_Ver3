...
void InitBullet(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\yellowgreenball_char.png",
		&g_pTextureBullet[0]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\redball_enemy.png",
		&g_pTextureBullet[1]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\brueball_char.png",
		&g_pTextureBullet[2]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\Skill_Slash.png",
		&g_pTextureBullet[3]);
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\orangeball_enemy.png",
		&g_pTextureBullet[5]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\arrow_bullet_000.png",
		&g_pTextureBullet[6]);
		...
		D3DXCreateTextureFromFile(pDevice,
			"data\\purpleball_enemy.png",
			&g_pTextureBullet[8]);

		D3DXCreateTextureFromFile(pDevice,
			"data\\yellowgreenball_enemy.png",
			&g_pTextureBullet[9]);

		D3DXCreateTextureFromFile(pDevice,
			"data\\wind_bullet_000.png",
			&g_pTextureBullet[10]);

		D3DXCreateTextureFromFile(pDevice,
			"data\\Reflection_Bullet.png",
			&g_pTextureBullet[11]);
...
void UpdateBullet(void)
{
	...
	{
		if (pPlayer->bInvincibility == true)
		{
			for (nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
			{
		...
		{//弾が使用されている
				...
				(g_aBullet[nCntBullet].pos.y >= SCREEN_HEIGHT) || (g_aBullet[nCntBullet].pos.y <= 0))//角にぶつかった
			{
				...
				if(g_aBullet[nCntBullet].type!=BULLETTYPE_ARROW)
				{
					if (g_aBullet[nCntBullet].type != BULLETTYPE_REFLECTION)
					{
						...
						{
							SetExplosion(g_aBullet[nCntBullet].pos, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), EXPLOSIONTYPE_NORMAL);

							if (g_aBullet[nCntBullet].type == BULLETTYPE_NOVA)
							{
								SetExplosion(g_aBullet[nCntBullet].pos, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), EXPLOSIONTYPE_NOVA);
							}
							g_aBullet[nCntBullet].bUse = false;//使用していない状態にする。
						}
					}
			...
			{//プレイヤーの弾
				...
				for (nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++, pEnemy++)
				{
					if (pEnemy->bUse == true)
					{//敵が使用されている
						if (pEnemy->nType == ENEMYTYPE_BOSS1_STARWARRIOR)
						{
							if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - 120.0f && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + 120.0f &&
								g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - 120.0f && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + 120.0f)
							{//敵と弾が当たった
						...
						else if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - 20.0f && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + 20.0f &&
							g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - 20.0f && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + 20.0f)
						{//敵と弾が当たった
							HitEnemy(nCntEnemy, 1.0f);
			...
			{//プレイヤーの弾
				...
				for (nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++, pEnemy++)
				{
					if (pEnemy->bUse == true)
					{//敵が使用されている
						if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - 30.0f && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + 30.0f &&
							g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - 30.0f && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + 30.0f)
						{//敵と弾が当たった
						...
						else if (pEnemy->nType == ENEMYTYPE_BOSS1_STARWARRIOR)
						{
							if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - 120.0f && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + 120.0f &&
								g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - 120.0f && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + 120.0f)
							{//敵と弾が当たった
			...
			{//敵の弾(敵全体に適用）
				...
				{//プレイヤーに弾が当たった
					...
					SetExplosion(pPlayer->pos, D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f),EXPLOSIONTYPE_NORMAL);
					//敵の弾を使用していない状態にする
					g_aBullet[nCntBullet].bUse = false;//弾を使用していない状態にする
			...
			{//エネミー５の弾
				...
				case 1://Easy
					...
					{
						g_aBullet[nCntBullet].fEnemy5Curve = (D3DX_PI / 2.0f) * g_aBullet[nCntBullet].nNumber + g_fRandumResultBullet2_B;
					}
					break;
				case 2://Normal
					if (g_aBullet[nCntBullet].nApperCount == 1)
					{
						g_aBullet[nCntBullet].fEnemy5Curve = (D3DX_PI / 4.0f) * g_aBullet[nCntBullet].nNumber + g_fRandumResultBullet2_B;
					}
					break;
				case 3://Normal
					if (g_aBullet[nCntBullet].nApperCount == 1)
					{
						g_aBullet[nCntBullet].fEnemy5Curve = (D3DX_PI / 7.0f) * g_aBullet[nCntBullet].nNumber + g_fRandumResultBullet2_B;
					}
					break;
				case 4://Normal
					if (g_aBullet[nCntBullet].nApperCount == 1)
					{
						g_aBullet[nCntBullet].fEnemy5Curve = (D3DX_PI / 10.0f) * g_aBullet[nCntBullet].nNumber + g_fRandumResultBullet2_B;
					}
					break;
				}
			...
			{//エネミー５の弾
				...
				g_aBullet[nCntBullet].fCurve -= 1.1f;
				switch (pGameCustom->nDifficulty)
				{
				case 1://Easy
					if (g_aBullet[nCntBullet].nApperCount == 1)
			...
			{//更に自機狙い弾を発射
				...
				if (g_nDelayBullet % 3 == 0)
				{
					...
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim + 0.15f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim + 0.15f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim - 0.15f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim - 0.15f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim + 0.3f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim + 0.3f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim - 0.3f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim - 0.3f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim + 0.45f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim + 0.45f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim - 0.45f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim - 0.45f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim + 0.6f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim + 0.6f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim - 0.6f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim - 0.6f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim + 0.75f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim + 0.75f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim - 0.75f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim - 0.75f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim + 0.9f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim + 0.9f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
					SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3(sinf(g_aBullet[nCntBullet].fVaim - 0.9f) * 15.0f, cosf(g_aBullet[nCntBullet].fVaim - 0.9f) * 15.0f, 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_BANDY, 6, -1, -1);
			...
			{//発射した弾から、さらにばらまく。
				...
				if (pGameCustom->nDifficulty == 4)
				{
					...
					{
						SetBullet(g_aBullet[nCntBullet].pos, D3DXVECTOR3((sinf(-D3DX_PI * (fRandumScattering_Rot)) * fRandumScattering_Speed), (cosf(-D3DX_PI * (fRandumScattering_Rot)) * fRandumScattering_Speed), 0.0f), GENERALLY_BULLETLIFE, BULLETTYPE_ENEMY, -1, -1, -1);
					}
				}
				else
				{
			...
			{//エネミー５の弾
				...
				//else if (g_aBullet[nCntBullet].nApperCount >= 100)
				//{
				//	//カーブを止める
				//}
				if (pGameCustom->nDifficulty == 4)
			...
			{//スキル３（Xアロー）
				...
				if (pGameCustom->nDifficulty == 4)
				{
					g_aBullet[nCntBullet].move += g_aBullet[nCntBullet].move * 0.004f;
				}
				else
				{
			...
			{//プレイヤーのスキル１
				...
				for (nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++, pEnemy++)
				{
					...
					{//敵が使用されている
						...
						{
							if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - BIGBULLET_LV1 && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + BIGBULLET_LV1 &&
								g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - BIGBULLET_LV1 && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + BIGBULLET_LV1)
							{//敵と弾が当たった
			...
			{//プレイヤーのスキル2
				...
				for (nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++, pEnemy++)
				{
					...
					{//敵が使用されている
						...
						{
							if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - BIGBULLET_LV2 && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + BIGBULLET_LV2 &&
								g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - BIGBULLET_LV2 && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + BIGBULLET_LV2)
							{//敵と弾が当たった
					...
					{//敵が使用されている
						if (pEnemy->nType == ENEMYTYPE_BOSS1_STARWARRIOR)
						{
							if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - 120.0f && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + 120.0f &&
								g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - 120.0f && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + 120.0f)
							{//敵と弾が当たった
								HitEnemy(nCntEnemy, 15.0f);//ボスに当たりすぎるので、ダメージを減らす。
						...
						{
						if (g_aBullet[nCntBullet].pos.x >= pEnemy->pos.x - 20.0f && g_aBullet[nCntBullet].pos.x <= pEnemy->pos.x + 20.0f &&
							g_aBullet[nCntBullet].pos.y >= pEnemy->pos.y - 20.0f && g_aBullet[nCntBullet].pos.y <= pEnemy->pos.y + 20.0f)
						{//敵と弾が当たった
...
//弾の設定処理
void SetBullet(D3DXVECTOR3 pos, D3DXVECTOR3 move,int nLife,BULLETTYPE type, int nNum,int nNumEnemy,int nAimingEnemy)
{
	int nCntBullet;
	...
	for (nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		...
		/*	if (BULLETTYPE_ENEMY || BULLETTYPE_PLAYER || BULLETTYPE_SKILL1||BULLETTYPE_REAFY)
			...
			/*	pVtx[0].pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
				...
				//if (g_aBullet[nCntBullet].type == BULLETTYPE_SKILL2_SLASH)
				//{
				//	/*g_fAnimationPattern_Slash[nCntBullet] = 0.0f;
				//	g_nAnimatioinCounter_Slash[nCntBullet] = 0;*/
