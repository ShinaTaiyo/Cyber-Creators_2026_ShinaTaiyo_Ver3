...
#define EXPLOSION_TEXTURE "data\\explosion000.png"
#define EXPLOSION_SCALE (20)
#define EXPLOSION_SCALE_NOVA (400.0f)//スキル「ノヴァブラスト」の爆発範囲
		...
		{//爆発が使用されている（true)の間は、ずっと処理される
			...
			if (g_aExplosion[nCntExplosion].nType == EXPLOSIONTYPE_NOVA)
			{
				...
				for (nCntBullet = 0; nCntBullet < MAX_ENEMY; nCntBullet++, pBullet++)
				{
					...
					{
						if (pBullet->type != BULLETTYPE_NOVA)
						{
							if (pBullet->pos.x >= g_aExplosion[nCntExplosion].pos.x - EXPLOSION_SCALE_NOVA && pBullet->pos.x <= g_aExplosion[nCntExplosion].pos.x + EXPLOSION_SCALE_NOVA &&
								pBullet->pos.y >= g_aExplosion[nCntExplosion].pos.y - EXPLOSION_SCALE_NOVA && pBullet->pos.y <= g_aExplosion[nCntExplosion].pos.y + EXPLOSION_SCALE_NOVA)
							{//敵と弾が当たった
