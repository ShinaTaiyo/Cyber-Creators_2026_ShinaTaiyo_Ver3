...
#define MAX_SCOREDIGIT (9)//ランクインした順位のスコアの最大桁数
...
#define SHORTMODE_RANKING_TEXT "data\\TextFile\\ShortMode_Ranking.txt" //ショートモードのランキングのテキストファイルを読み込む

#define EASYMODE_SHORT_RANKING_TEXT "data\\TextFile\\EasyMode_Ranking_Short.txt"
#define NORMALMODE_SHORT_RANKING_TEXT "data\\TextFile\\NormalMode_Ranking_Short.txt"
#define HARDMODE_SHORT_RANKING_TEXT "data\\TextFile\\HardMode_Ranking_Short.txt"
#define VERYHARDMODE_SHORT_RANKING_TEXT "data\\TextFile\\VertHardMode_Ranking_Short.txt"

#define EASYMODE_NORMAL_RANKING_TEXT "data\\TextFile\\EasyMode_Ranking_Normal.txt"
#define NORMALMODE_NORMAL_RANKING_TEXT "data\\TextFile\\NormalMode_Ranking_Normal.txt"
#define HARDMODE_NORMAL_RANKING_TEXT "data\\TextFile\\HardMode_Ranking_Normal.txt"
#define VERYHARDMODE_NORMAL_RANKING_TEXT "data\\TextFile\\VertHardMode_Ranking_Normal.txt"

#define RANKING_STAGE1 "data\\TEXTFILE\\Ranking_Stage1"
#define RANKING_STAGE2 "data\\TEXTFILE\\Ranking_Stage2"
#define RANKING_STAGE3 "data\\TEXTFILE\\Ranking_Stage3"
#define RANKING_STAGE4 "data\\TEXTFILE\\Ranking_Stage4"

//#define MAX_SCORERANK (40)
typedef struct
{
...
//int g_nScorePosTexU[MAX_RANK][MAX_SCOREDIGIT];//スコアの数字の座標
...
void InitRanking(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\ranking_bg_002.png",
		&g_pTextureRankBG);
	...
	SetStaging(STAGINGTYPE_RANKING_BG, -1, D3DXVECTOR3(SCREEN_WIDTH/2,SCREEN_HEIGHT/2, 0.0f),10000,ATTACKTYPE_PUNCH,-1.0f,500,0.0f,1000.0f,1000.0f,0.0f);
...
void UpdateRanking(void)
{
	...
	if (GetKeyboardTrigger(DIK_RETURN) == true || GetJoyPadTrigger(JOYKEY_START) == true)
	{
		...
		SetFade(MODE_TITLE);//タイトル画面をフェードアウト後に呼び出す
	}
...
void SetRanking(int nScore)
{
	...
	int* pChooseMode = GetChooseCommand();

	int* pChallengeStage = GetChallengeStage();
	...
	//情報処理から持ってきた処理
	//===============================================================
	pFile = fopen(RANKING_TEXT, "r");//ファイルの読み込み
	/*fclose(pFile);*/
	//if (*pChooseMode == 0)
	//{//ノーマルモードなら
	//	pFile = fopen(RANKING_TEXT, "r");//ファイルの読み込み
	//}
	//else if (*pChooseMode == 1)
	//{//体験版モードなら

	//	if (*pChallengeStage == CHALLENGESTAGE_1)
	//	{
	//		pFile = fopen(RANKING_STAGE1, "r");//ファイルの読み込み
	//	}
	//	else if (*pChallengeStage == CHALLENGESTAGE_2)
	//	{
	//		pFile = fopen(RANKING_STAGE2, "r");//ファイルの読み込み
	//	}
	//	else if (*pChallengeStage == CHALLENGESTAGE_3)
	//	{
	//		pFile = fopen(RANKING_STAGE3, "r");//ファイルの読み込み
	//	}
	//	else if (*pChallengeStage == CHALLENGESTAGE_4)
	//	{
	//		pFile = fopen(RANKING_STAGE4, "r");//ファイルの読み込み
	//	}
	...
	//if (*pChooseMode == 0)
	//{//ノーマルモードなら
	//	pFile = fopen(RANKING_TEXT, "w");//ファイルの読み込み
	//}
	//else if (*pChooseMode == 1)
	//{//体験版モードなら
	...
	//	if (*pChallengeStage == CHALLENGESTAGE_1)
	//	{
	//		pFile = fopen(RANKING_STAGE1, "w");//ファイルの読み込み
	//	}
	//	else if (*pChallengeStage == CHALLENGESTAGE_2)
	//	{
	//		pFile = fopen(RANKING_STAGE2, "w");//ファイルの読み込み
	//	}
	//	else if (*pChallengeStage == CHALLENGESTAGE_3)
	//	{
	//		pFile = fopen(RANKING_STAGE3, "w");//ファイルの読み込み
	//	}
	//	else if (*pChallengeStage == CHALLENGESTAGE_4)
	//	{
	//		pFile = fopen(RANKING_STAGE4, "w");//ファイルの読み込み
	//	}

	//}

	//======================================================================
	//難易度や長さによって　、書き出しを行うテキストファイルの種類を変える
	//======================================================================
	//if (pGameCustom->nWaveLength == 1 && pGameCustom->nDifficulty == 1)
	//{//EasyMode,Short
	//	pFile = fopen(EASYMODE_SHORT_RANKING_TEXT, "w");//読み込み
	//}
	//else if (pGameCustom->nWaveLength == 1 && pGameCustom->nDifficulty == 2)
	//{//NormalMode,Short
	//	pFile = fopen(NORMALMODE_SHORT_RANKING_TEXT, "w");//読み込み
	//}
	//else if (pGameCustom->nWaveLength == 1 && pGameCustom->nDifficulty == 3)
	//{//HardMode,Short
	//	pFile = fopen(HARDMODE_SHORT_RANKING_TEXT, "w");//読み込み
	//}
	//else if (pGameCustom->nWaveLength == 1 && pGameCustom->nDifficulty == 4)
	//{//VeryHardMode,Short
	//	pFile = fopen(VERYHARDMODE_SHORT_RANKING_TEXT, "w");//読み込み
	//}

	//else if (pGameCustom->nWaveLength == 2 && pGameCustom->nDifficulty == 1)
	//{//EasyMode,Normal
	//	pFile = fopen(EASYMODE_NORMAL_RANKING_TEXT, "w");//読み込み
	//}
	//else if (pGameCustom->nWaveLength == 2 && pGameCustom->nDifficulty == 2)
	//{//NormalMode,Normal
	//	pFile = fopen(NORMALMODE_NORMAL_RANKING_TEXT, "w");//読み込み
	//}
	//else if (pGameCustom->nWaveLength == 2 && pGameCustom->nDifficulty == 3)
	//{//HardMode,Normal
	//	pFile = fopen(HARDMODE_NORMAL_RANKING_TEXT, "w");//読み込み
	//}
	//else if (pGameCustom->nWaveLength == 2 && pGameCustom->nDifficulty == 4)
	//{//VeryHardMode,Normal
	//	pFile = fopen(VERYHARDMODE_NORMAL_RANKING_TEXT, "w");//読み込み
	//}
	//======================================================================================================
	...
	int nCntDigit;//桁数
	int nCntRank;//順位（行数）
	/*int nGetScore = nScore;*/

	for (nCntRank = 0; nCntRank < MAX_RANK; nCntRank++)//ランクインしたスコア５個分の読み込み。
	{
		for (nCntDigit = 0; nCntDigit < MAX_SCOREDIGIT; nCntDigit++)
		{
