...
//コンストラクタ
//========================================================================
CDamage::CDamage() : m_nDamage(0),m_pNumber3D(),m_Pos(NULL_VECTOR3),
...
CDamage* CDamage::Create(int nDamage, D3DXVECTOR3 Pos, D3DXCOLOR col, float fWidth, float fHeight)
{
	...
	{//生成が成功したら
		if (pDamage != nullptr)
		{
			...
			pDamage->CObject::SetType(CObject::TYPE_DAMAGE);                                                  //オブジェクトの種類を決める
			for (int nCnt = 0; nCnt < m_nMAX_DAMAGEDIGIT; nCnt++)
			{
				pDamage->m_pNumber3D[nCnt] = CNumber3D::Create(fWidth, fHeight, D3DXVECTOR3(Pos.x + fHalfWidth - fHalfWidth * nCnt, Pos.y + fHeight, 0.0f),D3DXVECTOR3(0.0f,12.0f,0.0f), col);
