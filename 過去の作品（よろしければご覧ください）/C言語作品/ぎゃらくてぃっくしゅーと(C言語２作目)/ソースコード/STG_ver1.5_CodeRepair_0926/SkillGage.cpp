...
void UpdateSkillGage(void)
{
	...
	for (nCntSkillGage = 0; nCntSkillGage < MAX_SKILLGAGE; nCntSkillGage++)
	{
		...
		else
		{
			...
			if (pSkill->fSkillPattern == 0)
			{//選んでいるスキルが「ビッグショット」の時
				g_aSkillGage[nCntSkillGage].fSkillGage = 50.0f;
			}
			else if (pSkill->fSkillPattern == 1)
			{//選んでいるスキルが「スラッシュ」の時
				g_aSkillGage[nCntSkillGage].fSkillGage = 100.0f;
			}
			else if (pSkill->fSkillPattern == 2)
			{//選んでいるスキルが「エックスアロー」の時
				g_aSkillGage[nCntSkillGage].fSkillGage = 180.0f;
			}
			else if (pSkill->fSkillPattern == 3)
			{//選んでいるスキルが「ノヴァブラスト」の時
				g_aSkillGage[nCntSkillGage].fSkillGage = 750.0f;
			}
			else if (pSkill->fSkillPattern == 4)
			{//選んでいるスキルが「ヒール」の時
				g_aSkillGage[nCntSkillGage].fSkillGage = 500.0f;
			}
			else if (pSkill->fSkillPattern == 5)
