...
void InitGage(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\BossHp_000.png",
		&g_apTextureGage[0]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\BossHp_frame_000.png",
		&g_apTextureGage[1]);
		...
		if (g_aGage[nCntGage].bUse == true)
		{
			...
			/*	if (g_aGage[nCntGage].bUse == true)
				{*/
					...
					pVtx[1].pos = D3DXVECTOR3(HPGAGE_LENGTH, 680.0f, 0.0f);
					pVtx[2].pos = D3DXVECTOR3(0.0f,SCREEN_HEIGHT, 0.0f);
					pVtx[3].pos = D3DXVECTOR3(HPGAGE_LENGTH,SCREEN_HEIGHT, 0.0f);
				...
				for (nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++, pEnemy++)
				{
					...
					{//ボス１が倒されたら・・・
						...
						if (pEnemy->bUse == true && g_aGage[nCntGage].nSubjectEnemy == nCntEnemy)
						{
							...
							pVtx[0].pos = D3DXVECTOR3(0.0f,680.0f, 0.0f);
							pVtx[1].pos = D3DXVECTOR3(0.0f + (HPGAGE_LENGTH / g_aGage[nCntGage].fBoss1MaxHp) * pEnemy->fLife, 680.0f, 0.0f);
							pVtx[2].pos = D3DXVECTOR3(0.0f,SCREEN_HEIGHT, 0.0f);
							pVtx[3].pos = D3DXVECTOR3(0.0f + (HPGAGE_LENGTH / g_aGage[nCntGage].fBoss1MaxHp) * pEnemy->fLife, SCREEN_HEIGHT, 0.0f);
